...
int main(int argc, char **argv)
{
    ...
    auto hfnet = pipeline.create<dai::node::NeuralNetwork>();
    ...
    hfnet->setBlobPath(nnPath);
    hfnet->setNumInferenceThreads(2);
    hfnet->setNumNCEPerInferenceThread(1);
    hfnet->input.setBlocking(false);
    ...
    manip->out.link(hfnet->input);
    hfnet->out.link(xoutNN->input);
    ...
    auto hfnetQueue = device.getOutputQueue("nn", 8, false);
    ...
    cv::namedWindow("HF-Net");
    cv::createTrackbar("Detector confidence threshold %", "HF-Net", &defaultValue, 100, updateConfThresh, &confThresh);
    ...
    while (true)
    {
        ...
        auto hfnetData = hfnetQueue->get<dai::NNData>();
        while (hfnetData->getSequenceNum() < left->getSequenceNum())
            hfnetData = hfnetQueue->get<dai::NNData>();
        ...
        cv::resize(fromPlanarFp16(hfnetData->getLayerFp16("pred/local_head/detector/Squeeze"), 320, 200, confThresh), heatmap, cv::Size(1280, 800));
